...
class tr_form extends tr_base {
  ...
  public function repeater($name, $fields, $settings = array()) {
    ...
    $templatesContainer = '<div class="repeater-controls"><div class="move tr-icon-menu"></div><a href="#remove" class="remove tr-icon-remove2" title="remove"></a></div><div class="repeater-inputs">';
...
public function gallerys($name, $attr = array(), $settings = array(), $label = true) {
    $field = new tr_field_gallerys();
    $this->setup_field($field, $name, $settings);
    $field->attr += $attr;
    $this->add_field($field, $settings, $label);
